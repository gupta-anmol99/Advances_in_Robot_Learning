...
try:
    while True:
        ...
        if depth_colormap_dim != detection_shape:
            detection = cv2.resize(detection, dsize=(depth_colormap_dim[1], depth_colormap_dim[0]), interpolation=cv2.INTER_AREA)

        images = np.hstack((detection, depth_colormap))
